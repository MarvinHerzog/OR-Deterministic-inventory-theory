...
#      300,350,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500,1600,1700,1800,1900,2000,2500,3000,3500,4000) #,4500,5000,5500,6000,6500,7000
# V=c()
# for(skal in G){
###
###
...
for (ind in zaporedje) {
  ...
  if (!is.na(yopt.force)) {
    ...
    diskontirana_cena = 0
    if ((q < yopt) & (diskontiranje == TRUE)) {
      diskontirana_cena = 1
      diskontiranje = FALSE
    }
   
    
    if (diskontiranje == TRUE) {
      Q = result(1,(2 * (c2*D - (  D * c1 + (K * D) / yopt + (h*yopt) / 2)      ) / h),(2 * K * D) / h)
      if ((Q[1] > yopt) & (q < Q[1]) & (q > yopt)) {
        yopt = q
        diskontirana_cena = 1
      }
      if ((Q[2] > yopt) & (q < Q[2]) & (q > yopt)) {
        yopt = q
        diskontirana_cena = 1
      }
    }
  ...
  ## skupen strosek je (sestevek pozitivnih ploscin)*(holding cost)+(sestevek negativnih ploscin)*(shortage cost)+(setup cost)*(st. narocil)+(stevilo izdelkov)*cena
  ...
  TC = sum(S.poz) * h + sum(S.neg) * p + (length(I[,1])) * K + sum(Stevilo_izdelkov) *
    diskontirana_cena * c2 + sum(Stevilo_izdelkov) * (1 - diskontirana_cena) *
    c1#skupni stroski
  percycle = S.poz * h + S.neg * p + K + Stevilo_izdelkov * diskontirana_cena *
    c2 + Stevilo_izdelkov * (1 - diskontirana_cena) * c1 #vektor stroskov po ciklih
...
# V=append(V,B3[3])
# }
# plot(x=G,y=V,xlab= "n",ylab="Cas izvajanja (s)")
# abline(lm(V ~ G))
####
...
if (TCtest != F) {
  ...
  plot(
    x = zaporedje,y = TCv / TCdef,xlab = paste("Sprememba variirane spremenljivke:",TCtest),ylab =
