...
for (ind in 1:length(H[,1])) {
  ...
  while (I[ci,1] <= n) {
    ...
    if (prepreci.propad & !is.na(I[ci + 1,2])) {
      ...
      if (I[ci + 1,2] < RP &
        ...
        supernarocilo = S+abs(I[ci + 1,2] - L*D + (j1-2)*yopt)
        indeksi.supernarocil=append(indeksi.supernarocil,cassupernarocilo)
        narocila = append(narocila,supernarocilo)
        zereseno = T
        x1 = t
      }
    }
  ...
  I2 = I
  I2[,3] = I2[,3] - yopt
  I2[indeksi.supernarocil,3] = I2[indeksi.supernarocil,3] + yopt - narocila
  I2[,1] = I2[,1] - eps1
  I2[,4] = "orderL"
